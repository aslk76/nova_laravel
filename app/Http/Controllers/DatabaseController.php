...
class DatabaseController extends Controller
{
    ...
    {
        if ($weekid == 1) {
            $items = Balanceops::whereNull('deleted_at')->where('command', '<>', 'Casino')->where('command', '<>', 'Lottery')->orderBy('id', 'DESC')->take(2000)->get();
            return $items;
        }
        if ($weekid == 2) {
            $week = DB::select("SELECT cur1, cur2 from variables");
        ...
        $items = Balanceops::whereNull('deleted_at')->whereBetween('date', [$startday, $endday])->where('command', '<>', 'Casino')->where('command', '<>', 'Lottery')->orderBy('id', 'DESC')->take(2000)->get();
        return $items;
    }
    ...
    {
        $items = RaidCollecting::where('collected', 1)->orderBy('updated_at', 'DESC')->take(2000)->get();
        return $items;
    }
    public function getMissingRaids()
    {
        $items = RaidCollecting::where('missing', 1)->orderBy('updated_at', 'DESC')->take(2000)->get();
        return $items;
    }
    ...
    public function getPaymentsRealm($realm)
    {
        ...
        WHERE booster LIKE '%".str_replace("'", "\\'", $realm)."' AND paymentdate BETWEEN (SELECT cur1 FROM `variables` WHERE id = 1) AND
        (SELECT cur2 FROM `variables` WHERE id = 1);");
